...
class AugmentationChain:
    ...
    def __call__(self, x):
        """
        Apply augmentation chain to audio in `x`.

        Args:
            x (Numpy array): Audio samples of shape `n_samples` x `n_channels`.

        Returns:
            y (Numpy array): Processed audio of same shape as `x` where effects have been applied.
        """
        ...
        for fx, p, rms_normalize in self.fxs:
            if np.random.rand() < p:
                if isinstance(fx, Processor):
                    ...
                    # apply processor
                    y = self.apply_processor(y, fx, rms_normalize)
                else:
                    # apply effect chain
                    y = fx(y)
...
class Equaliser(Processor):
    ...
    def process(self, x):
        ...
        """
        for _band, iirfilter in self.filters.items():
#             iirfilter.reset_state() Uncomment when replicating results is needed
...
def compressor_process(x, threshold, attack_time, release_time, ratio, makeup_gain, sample_rate, yL_prev):
    ...
    y_l = np.zeros(M)
    c = np.zeros(M)
#     yL_prev = 0. # uncomment when replicating results is needed
...
class Compressor(Processor):
    ...
    def __init__(self, sample_rate, name='Compressor', parameters=None):
        ...
        if not parameters:
            ...
            self.parameters.add(Parameter('attack_time', 2.0, 'float', units='ms', minimum=0.03, maximum=500.0))
            self.parameters.add(Parameter('release_time', 50.0, 'float', units='ms', minimum=50.0, maximum=1000.0))
            self.parameters.add(Parameter('ratio', 2.0, 'float', minimum=0.05, maximum=20.0))
...
class ConvolutionalReverb(Processor):
    ...
    def __init__(self, impulse_responses, sample_rate, name='ConvolutionalReverb', parameters=None):
        ...
        if not parameters:
            ...
            self.parameters.add(Parameter('wet', 1.0, 'float', minimum=0.1, maximum=1.0))
            self.parameters.add(Parameter('dry', 1.0, 'float', minimum=0.1, maximum=1.0))
...
#         self.h = np.copy(self.impulse_responses[self.parameters.index.value]['impulse_response'])
        self.h = np.copy(self.impulse_responses[self.parameters.index.value]['impulse_response']())
...
class AlgorithmicReverb(Processor):
    def __init__(self, name="algoreverb", parameters=None, sample_rate=44100, **kwargs):
        ...
        if not parameters:
            self.parameters = ParameterList()
            self.parameters.add(Parameter("bypass",    False, "bool", p=0.1))
